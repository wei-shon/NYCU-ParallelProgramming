...
void workerThreadStart(WorkerArgs *const args)
{
    ...
    // Cut the pictures to numThreads part and let the thread to run its part

    // double Start_time = CycleTimer::currentSeconds(); 
    // int height_thread = args->height / args -> numThreads;
    // int  startRow_thread = args -> threadId * height_thread;

    // mandelbrotSerial(args->x0 , args->y0 , args -> x1 , args->y1 , args -> width , args->height,
    // startRow_thread , height_thread , args->maxIterations , args -> output);
    
    // double End_time = CycleTimer::currentSeconds(); 
    // printf("Thread %d times: [%.3f]ms\n" , args->threadId , (End_time - Start_time)*1000);


    // Give thread to run one row by one row
