...
int main(int argc, char **argv)
{
    ...
    // TODO: use MPI_Gather
    int gather[world_size];
    MPI_Gather(&number_in_circle, 1, MPI_INT, gather, 1, MPI_INT, 0, MPI_COMM_WORLD);

    if (world_rank == 0)
    {
        // TODO: PI result
        for(int source = 1 ; source < world_size ; source++) number_in_circle += gather[source];
