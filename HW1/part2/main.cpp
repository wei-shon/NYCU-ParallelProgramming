...
extern void test2(float *__restrict a, float *__restrict b, float *__restrict c, int N, int count);
extern double test3(double* __restrict a, int N, int count) ;
...
int main(int argc, char * argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, "c:st:?", long_options, NULL)) != EOF) {

    switch (opt) {
      case 'c':
        count = atoi(optarg);
        if (count <= 0) {
          cout << "Error: Workload count is set to" << count << " (<0).\n";
          return -1;
        }
        break;
      case 's':
  ...
  switch (whichTestToRun) {
    ...
    case 2: test2(values1, values2, output, N , count); break;
    case 3: test3(values3, N , count); break;
  }
...
void usage(const char* progname) {
  ...
  printf("  -c  --count        Just run the count times function (Default = 1)\n");
